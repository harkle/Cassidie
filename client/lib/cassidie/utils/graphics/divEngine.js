var DivEngine = Class.create({
	    ...
	    for (var y = 0; y < this.levelData.dimensions.height; y++) {
	    	for (var x = this.levelData.dimensions.width-1; x >= 0; x--) {
	    		...
	    		if (tiletData.sprite != '') {
	    			...
	    			sprite.setAttribute('style', 'position:absolute;height:'+spriteData.height+'px;z-index:0;width:'+spriteData.width+'px;left:'+(left -spriteData.x)+'px;top:'+(top - spriteData.y)+'px;background:url('+image+');');	
	...
	setCursor: function(file) {
	    this.cursor.style.backgroundImage = 'url('+file+'.png)';
	},
	...
	setEntityBackgroundImage: function(id, file, animationID) {
